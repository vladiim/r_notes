...
# # for(i in 1:4) plot(g, layout=layout.fuchterman.reingold, margin=0)

# layout.old <- layout.fruchterman.reingold(g)
# for(i in 1:4) {
#   layout.new <- layout.fruchterman.reingold(g, params=list(niter=10,
#   	                                        maxdelta=2,
#   	                                        start=layout.old))
...
edges <- read.table('charts/edges.csv', header=TRUE)
